...
To build the Emacs module =autotools= is required. Once =autotools= is
available, run =make= in the top level directory and the module will be built
and the Emacs lisp files compiled. Alternatively, requiring the =zmq= library
will ask you to build the Emacs module if it cannot be found.
* Testing
