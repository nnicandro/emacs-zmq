        ...
        (with-zmq-socket sock zmq-PULL
          ...
          (ert-info ("String options")
            ...
            (should (equal (zmq-get-option sock zmq-PLAIN-PASSWORD)
                           "password"))
            ;; No multi-byte
            (should-error (zmq-set-option sock zmq-PLAIN-PASSWORD "paßword")
                          :type 'wrong-type-argument))
          ...
          (ert-info ("CURVE options")
            (when (zmq-has "curve")
              (cl-destructuring-bind (pub . priv)
                ...
                (zmq-set-option sock zmq-CURVE-SECRETKEY priv)
                ...
                (should (equal (zmq-get-option sock zmq-CURVE-PUBLICKEY)
                               pub))
                (should (equal (zmq-get-option sock zmq-CURVE-SECRETKEY)
                               priv))
